...
plt.setp(lines[1], linewidth=4.0, color='purple')
plt.setp(lines[2], linewidth=2.0,color='cyan')
plt.setp(lines[3], linewidth=2.0,color='green')
plt.legend(lines, ['radar SWE','SnoTEL SWE','SnoTEL precip','SnoTEL depth x 250 kg/m^3'])
plt.suptitle('Banner SWEdar thru: ' + str(t2[-1]) + ' and SnoTEL thru: ' + str(t[-1]) + '\n (c) 2017 CryoToolbox, LLC', fontsize=16)
plt.xlabel('Water Year 2018', fontsize=16)
plt.ylabel('water equivalent [cm]', fontsize=16)
A=plt.gca()
z=A.get_xlim()
